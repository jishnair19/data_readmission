...
library(xgboost)
classifier = xgboost(data = as.matrix(training_set[-16]), label = training_set$Exited, nrounds = 10)
...
cv = lapply(folds, function(x) {
  ...
  classifier = xgboost(data = as.matrix(training_fold[-16]), label = training_fold$Exited, nrounds = 10)
  y_pred = predict(classifier, newdata = as.matrix(test_fold[-16]))
  y_pred = (y_pred >= 0.5)
